...
Vercel uses the repository root, Framework Preset `Other`, install command `npm ci --include=dev`, build command `npm run build`, and output directory `apps/web/dist`. No Development Command is required. The explicit dev-dependency inclusion preserves the TypeScript/Vite build when `NODE_ENV=production` is present during installation. `api/index.js` imports the bundled `apps/server/dist/vercel.js` artifact; it must not import raw server or workspace TypeScript.
