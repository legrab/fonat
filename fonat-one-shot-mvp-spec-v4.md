...
## Authoritative One-Shot MVP Implementation Specification, Version 4
...
## 0.4 Authoritative toolchain, dependency, and artifact baseline

This section is a release gate, not a preference. It exists to prevent an implementation agent from producing a repository that works only inside its own temporary environment.
...
- `--force`, `--legacy-peer-deps`, `--ignore-engines`, or equivalent compatibility bypasses MUST NOT be the default fix for install errors. Resolve the incompatible package/version or document a narrow, reviewed exception.
- Existing `node_modules`, npm caches, globally installed CLIs, or generated build output MUST NOT be treated as evidence that the repository installs cleanly.

### Dependency selection and freshness

- Direct dependency versions MUST be selected from current official package or repository metadata at implementation time, not from model memory or an older scaffold. Use checks equivalent to `npm view <package> version engines peerDependencies` and the package's official release notes for major upgrades.
- Direct dependencies SHOULD be exact-pinned in `package.json`; the committed lockfile pins the transitive graph.
- Before finalizing, run `npm outdated` and `npm audit --omit=dev` where network access permits. Review results deliberately rather than performing an indiscriminate major-version upgrade.
- Dependencies that process untrusted archives, Markdown, HTML, SVG, authentication input, or network data require an explicit currency and compatibility check.
- If `archiver` is used for ZIP creation, use the current maintained stable major compatible with Node.js 24. At the time Version 4 was authored, the current release was `8.0.0`; do not silently fall back to remembered `5.x`, `6.x`, or `7.x` versions.
- `archiver` creates archives; it is not an archive-extraction safety boundary. ZIP import MUST use a maintained streaming reader plus Fonat-owned entry-count, expanded-size, depth, path-normalization, duplicate-name, symlink, compression, MIME, and SVG checks.
- If an exact preferred dependency cannot be installed in the agent environment, the agent MUST distinguish network/tooling failure from package incompatibility and record the limitation. It MUST NOT quietly pin an older package merely because it is cached.

### Portable package-lock and generated artifacts

- `package-lock.json` MUST be generated or normalized for ordinary external npm use. Remote `resolved` URLs MUST be publicly reachable and MUST NOT contain credentials, temporary agent registries, private mirrors, OpenAI/container hostnames, or machine-local paths. Prefer regenerating the lockfile against the public registry and proving it with `npm ci`; do not trust a blind text replacement of registry hosts.
- Reject lockfile references containing examples such as `applied-caas-gateway`, `.internal.`, `/mnt/data`, `/home/oai`, `/tmp/`, `file:///`, `localhost`, `127.0.0.1`, or absolute Windows paths.
- Relative npm workspace links are valid only when represented as normal workspace entries, typically with `link: true`; do not reject those as local contamination.
- The repository MUST include an automated lockfile-portability check. It SHOULD also scan committed source maps, generated manifests, logs, and packaged files for agent-local absolute paths and internal registry hosts.
- Before delivery, copy or extract the repository into a fresh directory outside the implementation working tree, remove `node_modules` and build output, run `npm ci`, rerun verification, start the production build, and create the final ZIP from that clean tree. After building, a production-runtime rehearsal SHOULD remove development dependencies with `npm prune --omit=dev` before starting the built server.
- The final ZIP MUST exclude `node_modules`, npm caches, Playwright browser caches, coverage, temporary databases, `.env` secrets, local logs, and agent-only files.

### Official reference points for this version

- Node.js release status and latest `24.x`: `https://nodejs.org/en/about/previous-releases` and `https://nodejs.org/dist/latest-v24.x/`.
...
- Node.js 24 LTS, exact patch aligned across local, CI, Docker, Vercel, and Render;
- exact npm version recorded in `packageManager`;
- TypeScript end to end;
- npm workspaces with committed portable lockfile and `npm ci`;
- React + Vite;
- React Router;
...
- the clean-room repository installs with Node.js 24 using `npm ci`;
- the committed lockfile passes the portability scan and contains no agent-local registry or path leakage;
...
- the built production server starts from generated output without importing workspace TypeScript source files or depending on development-only loaders;
- direct archive dependencies are current, Node.js 24-compatible, and used for the correct creation/extraction responsibility;
...
├─ scripts/
│  ├─ check-toolchain.mjs
│  ├─ check-lockfile-portability.mjs
...
- environment preflight: record Node/npm versions, registry, platform, architecture, Docker availability, and Playwright browser availability;
- establish Node.js 24 version files, exact npm `packageManager`, public-registry lockfile policy, and clean-install scripts before broad implementation;
- verify current official versions for security- and packaging-sensitive dependencies, including the chosen ZIP creation and extraction libraries;
...
**Checkpoint:** toolchain check, lockfile-portability check, clean install, build, and configuration tests pass on Node.js 24. If the environment temporarily blocks Node.js 24, record the fallback and keep a mandatory final Node.js 24 migration checkpoint.
...
- verify the current maintained ZIP creation and extraction dependencies against Node.js 24 before implementing package I/O;
- ZIP safety;
- staging;
- transactional apply;
- export/round trip;
...
## Stage 13: deployment and primary verification

- production builds;
- OpenAPI/frontend generated types;
- parse and validate Docker Compose, Vercel, Render, JSON, and YAML configuration before runtime tests;
- start the built server using the production entry point and prove health, static frontend delivery, login/session cookie, authenticated API access, and logout;
- in a clean copy, build and then remove development dependencies before the runtime smoke where the deployment model permits it, proving that production does not depend on `tsx`, TypeScript, Vite, or test-only packages;
- verify the production bundle does not depend on workspace `.ts` source files, `tsx`, or other development-only runtime loaders;
- Docker, Vercel, Render;
- README truthfulness;
- full browser gate;
- honest deviations.

**Checkpoint:** all available atomic checks pass independently. A monolithic `npm run validate` timeout is not proof of failure or success; preserve and report each constituent result.

## Stage 14: final Node.js 24 modernization and portable-artifact rehearsal

This is a deliberately separate last round after functional validation. It prevents toolchain modernization from becoming mixed with feature debugging.

1. Confirm the latest maintained Node.js `24.x` LTS patch available to the implementation environment.
2. Update every Node declaration together: `.nvmrc`, `.node-version`, `engines`, `packageManager` compatibility, Docker base image, CI, Vercel, Render, and README.
3. Re-check direct dependency currency and Node.js 24 compatibility. In particular, replace stale archive packages rather than suppressing engine or peer warnings.
4. Remove `node_modules` and generated build output. Regenerate or normalize `package-lock.json` using an ordinary public npm registry.
5. Run the lockfile and generated-path portability scans. Workspace links may remain; internal registry URLs and agent-local absolute paths may not.
6. Extract/copy the source into a fresh clean-room directory and run `npm ci` with no reliance on the original working tree.
7. Rerun format, lint, typecheck, unit, API, Mongo, browser, package, OpenAPI, production build, production smoke, Docker/config, and deployment-adapter checks as available.
8. Create the final ZIP only from the clean-room tree, extract that ZIP once more, and rerun at least toolchain, lockfile, install, build, and production smoke checks against the extracted artifact.
9. Record exact Node.js and npm versions, archive-library versions, commands, results, unavailable checks, and checksum in `docs/VERIFICATION.md`.

**Checkpoint:** the downloadable artifact, not merely the agent's original directory, installs and starts on Node.js 24 without private registry access or local path assumptions.
...
| Node.js 24 alignment | R | - | smoke | browser runtime | R |
| Lockfile/artifact portability | R | - | clean-install smoke | - | R |
| Production bundle independence | R | - | production smoke | - | R |
| Archive dependency currency and role | R | - | package smoke | - | R |
...
- Do not begin broad feature generation before the toolchain, registry, clean-install, and lockfile-portability preflight is established.
- Do not treat successful type checking as proof that the production bundle starts. Execute the built entry point after a clean build, and verify its runtime dependency closure.
- Do not treat a single combined validation command as the only evidence. Run and record atomic checks so a timeout does not hide which step passed or failed.
- Install or verify Playwright Chromium early. If browser acquisition or policy blocks execution, record the environmental blocker and do not imply E2E passed.
- Check Docker availability early. If unavailable, still run `docker compose config` or equivalent static validation and report runtime verification as unavailable.
...
1. run the toolchain preflight and prove the final repository targets Node.js 24 consistently;
2. run every available verification command as atomic steps and preserve their individual results;
3. use real MongoDB for persistence-specific behavior;
4. install/verify the Playwright browser early, then run the browser paths that prove navigation, Logout, Exercise creation, Presentation exit/completion, learner submission, response table, leaderboard, Assignment drafts, and Assessment delivery;
5. build the production frontend/server and execute the built production entry point, preferably after pruning development dependencies in a clean copy, proving it does not import workspace TypeScript or require development-only loaders;
6. parse deployment configuration and build/start Docker where available;
7. verify the Vercel adapter and Render configuration where tooling permits;
8. perform the separate final Node.js 24 modernization pass, even when earlier work temporarily used Node.js 22;
9. verify direct dependency currency, especially ZIP/archive packages, and do not retain an obsolete `archiver` version from memory or cache;
10. regenerate or normalize `package-lock.json` for public external npm use and fail on internal registry URLs, credentials, or agent-local paths;
11. verify a fresh clean-room copy and then the extracted final ZIP with `npm ci`, build, and production smoke checks;
12. report exact unverified items without implying they passed;
13. record all deviations and why they preserve higher-priority behavior.
...
The repository SHOULD copy this summary into `docs/requirements-v4.yaml` and map each ID to implementation evidence.
...
requirements:
  ...
  OPS-NODE24:
    must: "The final repository and all deployment surfaces use one exact tested Node.js 24 version."
    evidence: [toolchain, build, smoke]
  OPS-LOCK-PORTABLE:
    must: "The committed lockfile and generated artifacts contain no private agent registry URLs, credentials, or machine-local paths."
    evidence: [unit, clean-install, artifact-scan]
  OPS-PRODUCTION-SMOKE:
    must: "The built production server starts without workspace TypeScript source or development-only loaders and passes health/auth/static smoke checks."
    evidence: [build, api, smoke]
  OPS-ARCHIVE-DEPS:
    must: "ZIP creation and extraction use current Node.js 24-compatible maintained packages with explicit bounded safety controls."
    evidence: [dependency-review, unit, package-integration]
  OPS-CLEAN-ARTIFACT:
    must: "The extracted final ZIP installs, builds, and starts in a fresh directory using npm ci."
    evidence: [clean-install, build, smoke]
```

# Appendix E. Version 4 consolidation and implementation-hardening changes

Version 4 preserves the Version 3 product design and adds lessons from a complete one-shot implementation attempt. In particular, it:

- makes Node.js 24 LTS the authoritative final runtime and requires a separate final modernization pass when an agent initially falls back to Node.js 22;
- aligns Node, npm, Docker, CI, Vercel, Render, and documentation rather than allowing each surface to drift;
- requires dependency versions to be checked against official current metadata instead of model memory or cached scaffolds;
- calls out stale `archiver` selection specifically and separates ZIP creation from safe ZIP extraction;
- requires public, portable `package-lock.json` content and automated rejection of private agent registries, credentials, and container-local paths;
- adds clean-room install and extracted-final-ZIP verification;
- distinguishes typecheck/build success from actually starting the production bundle;
- requires production smoke checks for health, static delivery, authentication, authenticated access, and logout;
- adds explicit configuration parsing to catch YAML and deployment quoting errors before runtime;
- requires Playwright and Docker capability checks early, while preserving honest reporting when the execution environment blocks them;
- requires atomic validation evidence so a long combined command or timeout cannot obscure which checks passed;
- adds toolchain, lockfile, generated-path, dependency-baseline, production-smoke, and artifact-verification scripts/documentation;
- preserves the Version 3 Priority 0 usability gates, workflow contracts, data contracts, staged implementation order, fixtures, and evidence matrix.
